...
app.add_middleware(
    SessionMiddleware,
    secret_key=os.environ.get("SECRET_KEY", "marketpredict-demo-session-v1-change-in-production"),
    session_cookie="saudi_campaign_session",
    same_site="lax",
...
def _ensure_demo_user() -> int:
    """Create the public, non-admin demo account on every cold start if needed."""
    with connect() as con:
...
OBJECTIVE_AR = {
    ...
    "Traffic": "الزيارات",
    "Awareness": "الوعي",
    "Lead Generation": "العملاء المحتملون",
    "Engagement": "التفاعل",
    "App Installs": "تثبيت التطبيق",
}
...
def _user(request: Request):
    ...
    if not uid:
        return None
    with connect() as con:
        return con.execute("SELECT id,name,email,is_admin FROM users WHERE id=?", (uid,)).fetchone()
...
def _flash(request: Request, message: str, category: str = "info"):
    arr = request.session.setdefault("flashes", [])
    arr.append([category, message])
    request.session["flashes"] = arr[-8:]


def _pop_flashes(request: Request):
...
async def permission_handler(request: Request, exc: PermissionError):
    return templates.TemplateResponse(request, "error.html", _context(request, code=400, title="طلب غير صالح", message="تعذر التحقق من الطلب. أعد تحميل الصفحة وحاول مرة أخرى."), status_code=400)
...
async def register(request: Request):
    if request.method == "POST":
        ...
        _check_csrf(request, form.get("_csrf"))
        ...
        if not errors:
            try:
                ...
                if "UNIQUE" in str(exc).upper():
                    errors.append("هذا البريد مسجل مسبقًا.")
                else:
                    raise
        for e in errors:
            _flash(request, e, "error")
...
async def login(request: Request):
    if request.method == "POST":
        ...
        _flash(request, "بيانات الدخول غير صحيحة.", "error")
        return RedirectResponse(f"/login?next={request.query_params.get('next','')}", status_code=303)
...
async def dashboard(request: Request):
    ...
    if redirect:
        return redirect
    with connect() as con:
        recent = con.execute("SELECT * FROM predictions WHERE user_id=? ORDER BY id DESC LIMIT 6", (user["id"],)).fetchall()
        stats = con.execute(
            """SELECT COUNT(*) n, AVG(probability) avg_prob, SUM(predicted_success) successes,
               AVG(model_roc_auc) avg_auc FROM predictions WHERE user_id=?""", (user["id"],)
        ).fetchone()
        objectives = con.execute(
            "SELECT objective, COUNT(*) n, AVG(probability) avgp FROM predictions WHERE user_id=? GROUP BY objective ORDER BY n DESC",
            (user["id"],),
        ).fetchall()
...
async def predict_page(request: Request):
    ...
    if redirect:
        return redirect
    if request.method == "POST":
        _rate_limit(f"user:{user['id']}")
        form = dict(await request.form())
        _check_csrf(request, form.pop("_csrf", None))
        ...
        pid = save_prediction(user["id"], payload, result, provenance)
        request.session["last_prediction"] = {
            "id": pid,
            "objective": result["objective"],
            "probability": result["probability"],
            "predicted_success": int(result["predicted_success"]),
            "threshold": result["threshold"],
            "decision_strength": result["decision_strength"],
            "confidence_label": result.get("confidence_label", ""),
            "model_roc_auc": result.get("model_roc_auc"),
            "model_balanced_accuracy": result.get("model_balanced_accuracy"),
            "warnings": result.get("warnings", [])[:4],
            "recommendations": result.get("recommendations", [])[:4],
            "assessment": assessment_summary(payload),
        }
        return RedirectResponse(f"/result/{pid}", status_code=303)
...
async def prediction_result(request: Request, prediction_id: int):
    ...
    if redirect:
        return redirect
    with connect() as con:
        row = con.execute("SELECT * FROM predictions WHERE id=? AND user_id=?", (prediction_id, user["id"])).fetchone()
    if not row:
        fallback = request.session.get("last_prediction") or {}
        if int(fallback.get("id", -1)) != prediction_id:
            return templates.TemplateResponse(request, "error.html", _context(request, code=404, title="غير موجود", message="لم يتم العثور على التنبؤ المطلوب."), status_code=404)
        return templates.TemplateResponse(
            request, "result.html",
            _context(
                request, p=fallback, inputs={}, warnings=fallback.get("warnings", []),
                recommendations=fallback.get("recommendations", []), provenance={},
                assessment=fallback.get("assessment", []), serverless_fallback=True,
            ),
        )
    ...
    assessment = assessment_summary(inputs)
    return templates.TemplateResponse(request, "result.html", _context(request, p=row, inputs=inputs, warnings=warnings, recommendations=recs, provenance=provenance, assessment=assessment, serverless_fallback=False))
...
async def history(request: Request):
    ...
    if redirect:
        return redirect
    with connect() as con:
        rows = con.execute("SELECT * FROM predictions WHERE user_id=? ORDER BY id DESC LIMIT 250", (user["id"],)).fetchall()
...
async def history_csv(request: Request):
    ...
    if redirect:
        return redirect
    with connect() as con:
        rows = con.execute("SELECT id,objective,probability,predicted_success,threshold,decision_strength,confidence_label,model_roc_auc,created_at FROM predictions WHERE user_id=? ORDER BY id DESC", (user["id"],)).fetchall()
    out = io.StringIO()
    w = csv.writer(out)
    w.writerow(["id","objective","probability","predicted_success","threshold","decision_strength","confidence_label","model_roc_auc","created_at"])
    for r in rows:
        w.writerow(list(r))
...
async def admin(request: Request):
    ...
    if not user["is_admin"]:
        return templates.TemplateResponse(request, "error.html", _context(request, code=403, title="غير مصرح", message="هذه الصفحة مخصصة للإدارة."), status_code=403)
    with connect() as con:
        users = con.execute(
            """SELECT u.id,u.name,u.email,u.created_at,COUNT(p.id) predictions
               FROM users u LEFT JOIN predictions p ON p.user_id=u.id GROUP BY u.id ORDER BY u.id DESC"""
        ).fetchall()
        counts = con.execute("SELECT objective,COUNT(*) n,AVG(probability) avgp FROM predictions GROUP BY objective ORDER BY n DESC").fetchall()
...
async def api_schema():
    return JSONResponse({
        ...
        "platform_placements": schema["platform_placements"],
        "platform_content_types": schema["platform_content_types"],
        "objective_bidding_strategies": schema["objective_bidding_strategies"],
        "region_city": schema["region_city"],
        "objective_specific_required": schema["objective_specific_required"],
        "labels": schema.get("field_labels_ar", {}),
    })
...
async def api_predict(request: Request):
    ...
    try:
        ...
        return JSONResponse({"error":"validation_failed","details":details}, status_code=400)
    pid = save_prediction(user["id"], payload, result, provenance)
...
def seed_admin(email: str | None = None, password: str | None = None):
    email = (email or os.environ.get("ADMIN_EMAIL") or "admin@example.local").strip().lower()
    password = password or os.environ.get("ADMIN_PASSWORD")
    if not password:
        raise SystemExit("Set ADMIN_PASSWORD before seeding an admin.")
    with connect() as con:
