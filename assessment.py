...
def budget_adequacy_score(budget_sar: float, audience_size: float, objective: str):
...
def rubric_score(field:str,level:str):
    cfg=RUBRICS.get(field)
    if not cfg: raise ValueError(f"Unknown rubric field: {field}")
    for item in cfg["levels"]:
        if item["value"]==level:return float(item["score"]),{"method":"standardized pre-launch rubric","level":level,"label":item["label"],"description":item["desc"]}
    raise ValueError(f"Unsupported rubric level for {field}: {level}")

def apply_auto_assessment(payload:dict[str,Any]):
    ...
    for field in SCORE_FIELDS:
        ...
        if key in out and out.get(key) not in (None,""):
            score,meta=rubric_score(field,str(out[key]));out[field]=score;provenance[field]=meta
        out.pop(key,None)
    if out.get("Brand_Awareness_Score") in (None,""):
        ...
        if maturity in maturity_map:out["Brand_Awareness_Score"]=maturity_map[maturity];provenance["Brand_Awareness_Score"]={"method":"brand maturity fallback","brand_maturity":maturity}
    if str(out.pop("score_mode","auto"))=="manual":warnings.append("تم استخدام درجات يدوية لبعض عوامل التخطيط؛ يفضل وضع Auto Assessment للحصول على قياس موحد قابل للتكرار.")
    for key in list(out):
...
def assessment_summary(payload:dict[str,Any]):
