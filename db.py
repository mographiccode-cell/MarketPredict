    ...
    with connect() as con:
        con.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS predictions (
                ...
                decision_strength TEXT NOT NULL,
        ...
        migrations = {"confidence_label":"TEXT NOT NULL DEFAULT ''","model_balanced_accuracy":"REAL","recommendations_json":"TEXT NOT NULL DEFAULT '[]'","provenance_json":"TEXT NOT NULL DEFAULT '{}'"}
...
def save_prediction(user_id: int, payload: dict[str, Any], result: dict[str, Any], provenance: dict[str, Any] | None = None) -> int:
    with connect() as con:
        cur = con.execute("""INSERT INTO predictions
            (user_id, objective, probability, predicted_success, threshold, decision_strength,
             confidence_label, model_roc_auc, model_balanced_accuracy, inputs_json, warnings_json,
             recommendations_json, provenance_json)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",(
                user_id,result["objective"],result["probability"],int(result["predicted_success"]),result["threshold"],result["decision_strength"],result.get("confidence_label",""),result.get("model_roc_auc"),result.get("model_balanced_accuracy"),json.dumps(payload,ensure_ascii=False),json.dumps(result.get("warnings",[]),ensure_ascii=False),json.dumps(result.get("recommendations",[]),ensure_ascii=False),json.dumps(provenance or {},ensure_ascii=False)))
