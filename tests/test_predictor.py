...
SAMPLE=json.loads((ROOT/'sample_payloads/sample_valid.json').read_text(encoding='utf-8'))
...
def test_forbidden_post_launch_field_is_rejected(predictor):
    bad=copy.deepcopy(SAMPLE); bad['ROAS']=9.9
    with pytest.raises(InputValidationError) as exc:
        predictor.predict(bad)
    assert 'ROAS' in str(exc.value)

def test_out_of_range_score_is_rejected(predictor):
    bad=copy.deepcopy(SAMPLE); bad['Creative_Quality_Score']=150
    with pytest.raises(InputValidationError): predictor.predict(bad)

def test_unsupported_platform_is_rejected(predictor):
    bad=copy.deepcopy(SAMPLE); bad['Platform']='Unknown Network'
    with pytest.raises(InputValidationError): predictor.predict(bad)

def test_missing_required_field_is_rejected(predictor):
    bad=copy.deepcopy(SAMPLE); bad.pop('Budget_SAR',None)
    with pytest.raises(InputValidationError): predictor.predict(bad)
...
def test_all_six_objectives_from_v5_samples(predictor):
    ...
    for path in (ROOT/'sample_payloads').glob('*.json'):
        if path.name=='sample_valid.json':
            continue
